...
/*===============================================================*/
int verify_ts_EMMA(EMMA*d,unsigned long long int *min, unsigned long long int *max )
{
  unsigned long long int ts;

  ts=((unsigned long long)d->grif.timestamp_up&0x00ffffff)<<24;
  ts|=((unsigned long long)d->grif.timestamp&0x00ffffff);
  *min=ts;
  *max=ts;
  return (int)(*max-*min);
}
...
/*===============================================================*/
void display_EMMA(EMMA* d)
{
  printf("--->    EMMA HIT          <--------------------------------------\n");
  display_channel_legend();
  display_channel(1,&d->grif);
}
/*===============================================================*/
...
void display_caheader(caheader* d)
{
  ...
  printf("CFD        fold                   %4.4x\n",d->Tfold);
  printf("Time Stamp fold                   %4.4x\n",d->TSfold);  
  printf("Energy hit pattern                %16.16lx %16.16lx\n",d->EHP[1],d->EHP[0]);
  printf("CFD hit pattern                   %16.16lx %16.16lx\n",d->THP[1],d->THP[0]);
  printf("Time Stamp hit pattern            %16.16lx %16.16lx\n",d->TSHP[1],d->TSHP[0]);
