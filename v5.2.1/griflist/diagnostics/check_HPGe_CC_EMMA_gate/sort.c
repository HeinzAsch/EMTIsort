...
int main(int argc, char *argv[])
{
  ...
  if(argc!=3)
    {
      printf("griflist_check_HPGe_CC_EMMA_gate fragment_list map\n");
      exit(-1);
    }
   printf("Program sort HPGe CC EMMA gate for a list with identified triggers\n");
   ...
   if(h!=NULL) delete h;   
    ...
    h=(TH1D*)hl->Clone("TS");
    h->Multiply(hf);
    h->SetName("CC-EMMA TSdiff");
    h->SetTitle("CC-EMMA TSdiff");
    h->Draw("HIST");
    gPad->SetLogy();
    theApp->Run(kTRUE);
}
