...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("griflist_trig_tsns_fold fragment_list\n");
      exit(-1);
    }
   printf("Program sorts spectrum of a fold for fragments with exactly the same trigger time stamp on a list\n");
	...
	while(1)
	  {
	    if(fread(&curr,son,1,fl)!=1) break; //stop on incorrect reading
	    if(curr.trig_tsns>0)//new event	      
	      if(curr.trig_tsns!=ref.trig_tsns)//new event
		{
		  memcpy(&ref,&curr,son);
		  break;
		}
