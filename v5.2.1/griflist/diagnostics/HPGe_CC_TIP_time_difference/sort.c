...
int main(int argc, char *argv[])
{
  ...
  if(argc!=6)
    {
      printf("griflist_HPGe_CC_TIP_time_difference griflist_input_data map window_[ns] E_gate_low E_gate_high\n");
      exit(-1);
    }
  ...
  while(1)
      ...
      if(CC_channel(&current,&map)==1)
	{
	  if(current.tig_trig<1)
	    {
	      printf("Incorrect TIGRESS CC trigger %d at tsns %16lld channel %d. Exiting\n",current.tig_trig,current.tsns,current.chan);
	      exit(0);
	    }
	  
	  if(current.tig_trig!=CC_trig_store)
	    {
	      CC_trig_store=current.tig_trig;
