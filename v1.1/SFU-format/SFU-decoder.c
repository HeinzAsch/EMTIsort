...
int process_event(int* e, raw_event* data)
{
  ...
  while(ep<end)
	...
	case EMMA_TAG:
	  ep++; s=*ep-2;ep++;
	  memcpy(&data->emma,ep,s*sizeof(int));
	  ep+=s;
	  break;	  
	case GRIFFIN_TAG:
	  ep++; s=*ep-2;ep++;
	  ...
	  for(m=0;m<n;m++)
	    {
		  ...
		  for(int i=0;i<4;i++)
		    printf("Energy hit pattern %1d              %12.12lx\n",i,data->csiarray.h.EHP[i]);
		  for(int i=0;i<4;i++)
		    printf("CFD hit pattern %1d                 %12.12lx\n",i,data->csiarray.h.THP[i]);
		  for(int i=0;i<4;i++)
		    printf("Time Stamp hit pattern %1d          %12.12lx\n",i,data->csiarray.h.TSHP[i]);
