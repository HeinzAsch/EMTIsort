...
int main(int argc, char *argv[])
{
  ...
  if(argc!=4)
    {
      printf("griflist_TIP_EMMA_time_difference griflist_input_data map window_[ns]\n");
      exit(-1);
    }
  
 
  printf("Program sorts time difference histogram between EMMA and TIP fragments within a time window on the list\n");
  ...
  if(h!=NULL) delete h;
  h=new TH1D("TSdiff","TSdiff",window,-window,window);
  if(c!=NULL) delete c;
  ...
  if((output=fopen("EMMA_TIP_dt.mca","w"))==NULL)
    {
