		...
		if((int)diff<=time_high)
		  if(curr->ch.charge>=Egate_low)
			...
			if(Estore<=Egate_high)
			  {
			    h->Fill(curr->ch.charge/map->tig_kpar,Estore/map->tig_kpar);
			    //h->Fill(Estore/map->tig_kpar,curr->ch.charge/map->tig_kpar);
			  }
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=7)
    {
      printf("griflist_HPGe_CC_time_difference griflist_input_data map time_low time_high E_gate_low E_gate_high\n");
      exit(-1);
    }
  ...
  Egate_low=atoi(argv[5]);
  Egate_high=atoi(argv[6]);
  read_map(argv[2],&map);
