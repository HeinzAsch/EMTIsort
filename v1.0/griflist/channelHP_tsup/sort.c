...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("griflist_channelHP_tsup griflist_input_data [number]\n");
      exit(-1);
    }
  ...
  if((output=fopen("griflist_hitpattern.spn","w"))==NULL)
    {
  ...
  if((output=fopen("griflist_hitpattern_tsup0.spn","w"))==NULL)
    {
      printf("ERROR!!! I cannot open the mca file.\n");
      exit(EXIT_FAILURE);
    }
