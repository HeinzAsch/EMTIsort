...
int main(int argc, char *argv[])
{
  ...
  if(argc!=4)
    {
      printf("griflist_HPGe_CC_Seg_time_energy griflist_input_data map window_[ns]\n");
      exit(-1);
    }  
  start=clock();
      ...
      if(CC_channel(&current,&map)==1)
	{
	  ...
	  while(1)
		...
		if(same_HPGe(&current,&next,&map)==1)
		  {
		    h->Fill(current.ch.charge/map.tig_kpar,(int)dt);
		    h2->Fill(next.ch.charge/map.tig_kpar,(int)dt);
		  }
	      counter++;
	    }
      ...
      if(Seg_channel(&current,&map)==1)
	{
	  ...
	  while(1)
	      ...
	      if(CC_channel(&next,&map)==1)
		{
		  ...
		  if(same_HPGe(&current,&next,&map)==1)
		    {
		      h->Fill(next.ch.charge/map.tig_kpar,-(int)dt);
		      h2->Fill(current.ch.charge/map.tig_kpar,-(int)dt);
		    }
		}
	      counter++;
	    }
  ...
  TFile f("tsdiff_vs_charge_cc-seg.root","recreate");
